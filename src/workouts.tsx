        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[150] flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-[#121212] border border-[#2a2a2a] w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
              ...
              <form id="assign-form" onSubmit={handleAssignSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-[#1a1a1a] p-4 rounded-xl border border-[#2a2a2a]">
                    ...
                    {showClientDropdown && (
                      <div className="absolute z-10 w-full mt-1 bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg max-h-40 overflow-y-auto shadow-2xl custom-scrollbar">
                        {filteredClients.map(c => (
                          <div key={c.id} onClick={() => { setAssignForm({...assignForm, user_id: c.id}); setClientSearchTerm(`${c.first_name} ${c.last_name}`); setShowClientDropdown(false); }} className="p-2 hover:bg-[#2a2a2a] cursor-pointer text-white text-sm border-b border-[#2a2a2a] last:border-b-0">
                  ...
                  <div>
                    ...
                    <input type="date" required value={assignForm.date} onChange={(e) => setAssignForm({...assignForm, date: e.target.value})} className="w-full bg-[#121212] border border-[#2a2a2a] p-2.5 rounded-lg text-white outline-none text-sm focus:border-[#E31C25] transition-colors [color-scheme:dark]" />
                  </div>
                </div>
            ...
            <div className="p-5 border-t border-[#2a2a2a] bg-[#1a1a1a] shrink-0">
              <button form="assign-form" type="submit" disabled={isSubmitting || !assignForm.user_id} className={`w-full font-bold py-3 rounded-xl transition-colors flex items-center justify-center gap-2 ${isSubmitting || !assignForm.user_id ? 'bg-[#2a2a2a] text-gray-500 cursor-not-allowed' : 'bg-[#E31C25] text-white hover:bg-[#A6151B] shadow-[0_0_15px_rgba(227,28,37,0.3)]'}`}>
            ...
            <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
              <form id="exercise-form" onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="grid grid-cols-3 gap-4 p-4 bg-[#1a1a1a] rounded-xl border border-[#2a2a2a]">
                  ...
                  <div><label className="block text-[10px] uppercase font-bold text-gray-400 mb-1 text-center">Tiempo (s)</label><input type="number" required value={formData.time_estimate} onChange={(e) => setFormData({...formData, time_estimate: parseInt(e.target.value)})} className="w-full bg-[#121212] border border-[#2a2a2a] p-2 rounded-lg text-white text-center outline-none focus:border-[#E31C25] transition-colors" /></div>
                  <div><label className="block text-[10px] uppercase font-bold text-gray-400 mb-1 text-center">Descanso (s)</label><input type="number" required value={formData.rest_time} onChange={(e) => setFormData({...formData, rest_time: parseInt(e.target.value)})} className="w-full bg-[#121212] border border-[#2a2a2a] p-2 rounded-lg text-white text-center outline-none focus:border-[#E31C25] transition-colors" /></div>
                </div>

                <div><label className="block text-sm font-bold text-gray-400 mb-2">Descripción</label><textarea rows={4} value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} className="w-full bg-[#1a1a1a] border border-[#2a2a2a] p-3 rounded-xl text-white focus:border-[#E31C25] outline-none resize-none" /></div>
              </form>
            </div>
